...
export default function NFCBindingScreen({ navigation }) { 
  ...
  useEffect(() => {
    const loadUsuarios = async () => {
      setLoading(true);
      try {
        // Consultamos campos técnicos definidos en Odoo incluyendo la foto
  ...
  // Función de navegación corregida para ir al Escáner
  const handleVincular = (usuario) => {
    navigation.navigate('Control', { 
      screen: 'Escáner', // Nombre de la pantalla dentro de GuardiaStack
      params: { 
        // Pasamos los datos para que el escáner se ponga en "Modo Vinculación"
        studentToValidate: {
          ...
          esVinculacion: true // Flag opcional para lógica interna del escáner
        } 
      }
    });
  };
