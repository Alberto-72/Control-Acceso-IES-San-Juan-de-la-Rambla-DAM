...
// --- VISTA DE VINCULACIÓN NFC ---
const NFCBindingView = ({ setView, setTargetUser }) => {
  ...
  useEffect(() => {
    const loadAlumnos = async () => {
      setLoading(true);
      // Solicitamos campos técnicos definidos en Odoo: uid, nombre, curso y foto
      const result = await fetchOdooData('gestion_entrada.alumno', ["uid", "name", "surname", "school_year", "photo"]);
  ...
  const iniciarVinculacion = (alumno) => {
    // 1. Guardamos el alumno que queremos vincular en el estado global del Dashboard
    setTargetUser(alumno);
    // 2. Cambiamos automáticamente a la vista de Guardia (donde reside el escáner)
    setView('guardia');
    console.log("Iniciando modo vinculación para:", alumno.name);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-50">
          <h3 className="font-bold text-slate-700">Usuarios pendientes (Total: {alumnosSinNFC.length})</h3>
        </div>
        <div className="divide-y divide-slate-50">
          {loading ? (
            ...
            alumnosSinNFC.map((alum, i) => (
              <div key={i} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                <div className="flex items-center gap-3">
                  {/* Miniatura de foto del alumno proveniente de Odoo */}
                  <div className="w-10 h-10 rounded-full bg-slate-100 overflow-hidden flex items-center justify-center border border-slate-200">
                    {alum.photo ? (
                      <img src={`data:image/png;base64,${alum.photo}`} className="w-full h-full object-cover" alt="Avatar" />
                    ) : (
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold uppercase">{alum.school_year}</p>
                  </div>
                </div>
                <button 
                  onClick={() => iniciarVinculacion(alum)}
                  className="bg-blue-50 text-blue-600 px-4 py-2 rounded-xl text-xs font-bold border border-blue-100 hover:bg-blue-600 hover:text-white transition-all"
                >
                  Vincular
                </button>
              </div>
            ...
            <div className="p-10 text-center text-slate-400 font-bold uppercase text-xs">Sin alumnos pendientes.</div>
          )}
        </div>
      </div>
    </div>
...
export default function Dashboard({ user, onLogout }) {
  ...
  // Estado para capturar al alumno seleccionado para vinculación
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-900">
          ...
          <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-100 p-6 flex flex-col transition-transform duration-300 transform lg:translate-x-0 lg:static lg:h-screen lg:sticky lg:top-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="flex items-center gap-3 mb-12 px-2">
              ...
              <h1 className="font-bold text-slate-800 text-lg leading-tight tracking-tighter">
                ...
                <span className="text-blue-700 tracking-tighter">IES San Juan de la rambla</span>
              </h1>
            </div>
            
            <nav className="space-y-1 flex-1">
              <SidebarItem icon={BarChart3} label="Estadísticas" active={view === 'dashboard'} onClick={() => { setView('dashboard'); setIsSidebarOpen(false); setTargetUser(null); }} />
              <SidebarItem icon={Users} label="Alumnado" active={view === 'alumnado'} onClick={() => { setView('alumnado'); setIsSidebarOpen(false); setTargetUser(null); }} />
              <SidebarItem icon={UserSquare2} label="Profesorado" active={view === 'profesorado'} onClick={() => { setView('profesorado'); setIsSidebarOpen(false); setTargetUser(null); }} />
      ...
      {/* CONTENIDO PRINCIPAL */}
      <main className={`flex-1 p-6 md:p-10 overflow-y-auto w-full text-slate-900`}>
        ...
        <header className="flex justify-between items-center mb-8">
          ...
          {user.is_management && (
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="lg:hidden p-2 bg-white border border-slate-200 rounded-xl shadow-sm">
        ...
        {view === 'nfc' && <NFCBindingView setView={setView} setTargetUser={setTargetUser} />}
        
        {view === 'guardia' && (
          <div className="flex flex-col items-center justify-center min-h-[400px] bg-white rounded-3xl border-2 border-dashed border-slate-200 p-12 text-center">
            {targetUser ? (
              <>
                {/* Interfaz de espera para vinculación */}
                <div className="bg-blue-50 text-blue-600 p-6 rounded-full mb-6 animate-bounce"><Scan size={64} /></div>
                <h1 className="text-3xl font-black text-slate-800">Esperando Tarjeta...</h1>
                <p className="text-slate-500 font-bold mt-2">
                   VINCULANDO A: <span className="text-blue-600 uppercase underline decoration-2 offset-2">{targetUser.name} {targetUser.surname}</span>
                </p>
                <button 
                  onClick={() => setTargetUser(null)} 
                  className="mt-8 px-6 py-2 bg-red-50 text-red-500 rounded-full text-xs font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all shadow-sm"
                >
